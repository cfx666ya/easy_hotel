...
export default function CitySelectPage() {
  ...
  const scrollToLetter = (letter) => {
    ...
    if (container && target) {
      // target.offsetTop 表示【这个字母标题距离滚动容器顶部的距离】
      // 将容器滚动到该 letter 对应的 DOM 在滚动容器区域中的高度
      container.scrollTo({
      ...
      >
        ...
        >
          ...
          <div className="hot-city">
            <h3 ref={(el) => (letterRefs.current['热门'] = el)}>热门城市</h3>
          ...
          <div className="city-list">
            {Object.keys(cityGroups).map((letter) => (
              <div key={letter}>
                <h4 ref={(el) => (letterRefs.current[letter] = el)}>
                  {letter}
                </h4>
          ...
          {['热门', ...Object.keys(cityGroups)].map((letter) => (
            <span
              ...
              style={{
                padding: '3px 0',
                cursor: 'pointer',
                color: activeLetter === letter ? '#1890ff' : '#333',
